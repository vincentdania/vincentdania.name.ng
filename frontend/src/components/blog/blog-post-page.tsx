...
export async function generateBlogPostMetadata(slug: string): Promise<Metadata> {
  try {
    const [siteData, article] = await Promise.all([fetchSitePayload(), fetchArticle(slug)]);
            ...
            <aside className="space-y-5 xl:sticky xl:top-28 xl:self-start">
              ...
              <Card className="bg-white/92">
                <CardContent className="space-y-5">
                  <div className="space-y-3">
                    <p className="eyebrow">{blog.index_badge_label}</p>
                    <h3 className="font-display text-3xl leading-tight text-foreground">
                      {blog.detail_meta_heading}
                    </h3>
                  </div>
